...
public class GloVe {
    ...
    public static Vocabulary build_vocabulary(String file, Options options) throws Exception {
    ...
    public static List<Cooccurrence> build_cooccurrence(Vocabulary vocab, String corpusFile, Options options) throws Exception {
            ...
            while (iterator.hasNext()) {
                ...
                for (int center_i = 0; center_i < token_ids.size(); center_i++) {
                    ...
                    for (int left_i = 0; left_i < context_len; left_i++) {
                        ...
                        switch(options.weightFCode) {
                        case 1: increment = 1.0;
                        		break;
                        case 2: increment = 1.0/distance;
                        		break;
                        case 3: increment = 1.0/(distance*distance);
                        		break;
                        }
	...
	/*
	 * public static HashMap<OrderedIntegerPair, Double>
	 * build_cooccurrence2(Vocabulary vocab, String corpusFile, Options options) {
	 * 
	 * HashMap<OrderedIntegerPair, Double> cooccurrence_map = new
	 * HashMap<OrderedIntegerPair, Double>(vocab.getSize() * 2);
	 * 
	 * try { BigFile stream = new BigFile(corpusFile); Iterator<String> iterator =
	 * stream.iterator(); while (iterator.hasNext()) { String line =
	 * iterator.next(); String[] tokens = line.split(" ");
	 * 
	 * List<Integer> token_ids = vocab.recoverIds(tokens);
	 * 
	 * //iterate over each element for (int center_i = 0; center_i <
	 * token_ids.size(); center_i++) {
	 * 
	 * int center_id = token_ids.get(center_i);
	 * 
	 * int start = Math.max(0, center_i - options.window_size); int end = center_i;
	 * 
	 * //check if (end < start) { continue; }
	 * 
	 * List<Integer> context_ids = Methods.getSubset(token_ids, start, end); int
	 * context_len = context_ids.size();
	 * 
	 * for (int left_i = 0; left_i < context_len; left_i++) {
	 * 
	 * Integer left_id = context_ids.get(left_i);
	 * 
	 * int distance = context_len - left_i; double increment = 1.0 / distance;
	 * OrderedIntegerPair pair1 = new OrderedIntegerPair(center_id, left_id); Double
	 * get = cooccurrence_map.get(pair1); if (get == null) { get = 0.0; }
	 * 
	 * get += increment; cooccurrence_map.put(pair1, get);
	 * 
	 * OrderedIntegerPair pair2 = new OrderedIntegerPair(left_id, center_id); get =
	 * cooccurrence_map.get(pair2); if (get == null) { get = 0.0; }
	 * 
	 * get += increment; cooccurrence_map.put(pair2, get);
	 * 
	 * } } }
	 * 
	 * } catch (Exception ex) {
	 * Logger.getLogger(GloVe.class.getName()).log(Level.SEVERE, null, ex); }
	 * 
	 * List<Cooccurrence> coocurrences = new ArrayList<>(); for
	 * (Entry<OrderedIntegerPair, Double> entry : cooccurrence_map.entrySet()) {
	 * 
	 * Cooccurrence c = new Cooccurrence(entry.getKey().getWord1(),
	 * entry.getKey().getWord2(), entry.getValue());
	 * 
	 * coocurrences.add(c); }
	 * 
    ...
    public static DoubleMatrix train(Vocabulary vocab, List<Cooccurrence> cooccurrences, Options options) {
        ...
        for (int iteration = 1; iteration <= options.iterations; iteration++) {
            ...
            for (Cooccurrence c : cooccurrences) {
                ...
                double grad_bias_main = weight *cost_inner;
                double grad_bias_context = weight*cost_inner;
